...
func miseEnPlace(nom : String, roi : Carte) throws -> Joueur {
  ...
  if !carte.estRoi() { // cf : Carte -> ligne 100
    throw ShouldNotHappenError.ShouldBeKing
  }

  let joueur : Joueur = try! Joueur(nom : nom, roi : Carte) // cf : Joueur -> ligne 16
  try! piocher(nbr : 3, versRoyaume : false) // cf : Joueur -> ligne 52
  try! piocher(nbr : 1, versRoyaume : true)
...
func tour(joueur : Joueur, adversaire : Joueur) -> (Bool,Joueur?) {
  ...
  try! piocher(nbr : 1, versRoyaume : false) // cf : Joueur -> ligne 52

  // Phase d'action
  var finDeLaPartie : Bool = try! action(j : joueur, a : adversaire) // cf : mainDetaille -> ligne 98
  ...
  if !m.estVide() && !finDeLaPartie { // cf : Main -> ligne 21
    ...
    if m.estPleine() { // cf : Main -> ligne 27
      ...
      while !estReponse {
        let rep : String? = readLine("Désires-tu démobiliser une carte de ta Main vers ton Royaume ? (oui/non)")
        if let r==rep {
    ...
    if oui {
      let m : Main = joueur.getMain()
      let c : Carte = m.choisirMain(joueur : joueur, aPlacerOu : "dans le Royaume") // cf : bases -> ligne 17
...
func action(j : Joueur, a : Joueur) throws -> Bool {
  ...
  var estReponse : Bool = false
  while !estReponse {
    let rep : String? = readLine("Ecris le chiffre correspondant à ton choix (1, 2 ou 3)")
    if let r==rep {
  ...
  if r=="1" || r=="un" {
    let c : Carte = choisirMain(joueur : adv, aPlacerOu : "sur le front") // cf : bases -> ligne 17
    poserFront(joueur : adv, carte : c) // cf : bases -> ligne 48
  }
  ...
  // 2 - Attaquer
    ...
    while veutAttaquer && !finDeLaPartie {
      ...
      estReponse = false
      while !estReponse {
        let rep : String? = readLine("Choisis la position et donc la carte avec laquelle tu désires attaquer (A1,f2...)")
        if let r1==rep {
          if estPosition(p :r1) { // cf : bases -> ligne 139
            ...
            if caseVide {
              ...
              if f2.peutAttaquer(c1 : Carte, positionC : String) { // cf : Front -> ligne 45
                estReponse = true
              }
              else {
      ...
      afficherPortee(c : c, pos : r, a : a) // cf : bases -> 295

      // Choix de la cible
      estReponse = false
      while !estReponse {
        let rep2 : String? = readLine("Choisis la position et donc la carte que tu désires attaquer (A1,f2...)")
        if let r2==rep2 {
          if estPosition(p :r2) {
      ...
      estReponse = false
      while !estReponse {
        let rep : String? = readLine("Veux-tu encore attaquer ? (oui/non)")
        if let r==rep {
          if (r=="oui" || r=="o" || r=="OUI" || r=="Oui" || r=="ok" || r=="OK" || r=="Ok") {
            estReponse = true
          }
...
func attaquer(j : Joueur, cAtt : Carte, adv : Joueur, cAdv : Carte) -> Bool {
  ...
  let m1 : Front = j.getMain() // cf : Joueur -> ligne 34
  var pbl : Bool = cAdv.estRoi() // cf : Carte -> ligne 100

  if cAtt.getAtt(main : main) == cAdv.getDef() && cAdv.getDeg() == 0 { // cf : Carte -> lignes 46, 64 et 76
    j.capturer(adversaire : adv, carte : cAdv) // cf : Joueur -> ligne 70
  }
  else if cAtt.getAtt(main : main) + cAdv.getDeg() >= cAdv.getDef() {
    f2.supprimerCarteFront(carte : cAdv) // cf : Front -> ligne 57
  }
  else {
    pbl=false
    try! cAdv.setDeg(cAtt.getAtt(main : main) + cAdv.getDeg()) // cf : Carte -> ligne 82
  }
  return pbl
}
...
func conscription(adv : Joueur) throws -> Bool {
  ...
  if f.estVide() {                                              // cf : Front  -> ligne 27
    ...
    if !r.estVide() {                                           // cf : Royaume-> ligne 14
      ...
      //                                                        // cf : Royaume-> ligne 20
      afficherStat(c : c)                                       // cf : bases  -> ligne 385
