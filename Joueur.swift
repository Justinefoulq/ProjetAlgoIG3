...
class Joueur : JoueurProtocol {
    ...
    mutating func mobiliser (carte : Carte?, position : String) throws {
		var carteCourante : Carte
		var positionMobiliser : String

		if self.front.estLibre(position : position) {
					switch position { //On vérifie où positionner la carte
						case "F1":
							if self.front.estLibre(position : "A1") {
								positionMobiliser = "A1"
							}

						case "F2":
							if self.front.estLibre(position : "A2") {
								positionMobiliser = "A2"
							}

						case "F3":
							if self.front.estLibre(position : "A3") {
								positionMobiliser = "A3"
							}
		...
		if carte == nil { //On place la première carte du royaume du joueur
			if !self.royaume.estVide() {
				...
				self.front.ajouterCarteFront(position : positionMobiliser, carte : carteCourante)
				self.royaume.supprimerCarteRoyaume()
			}
		}
		else { //On place la carte de la main du joueur donné en paramètre
			self.front.ajouterCarteFront(position : positionMobiliser, carte : carte)
    ...
    // post : Ne renvoie rien
    mutating func capturer (adversaire : Joueur, carte : Carte) {
    	for carteFrontEnnemie in ItFront {
    		if carteFrontEnnemie == carte {
    			self.royaume.ajouterCarteRoyaume(carte : carte)
    			adversaire.front.supprimerCarteFront(carte : carte)
    		}
    	}
    }
}